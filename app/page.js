...
const CategorySelectionScreen = ({ onSelectCategory, onLogout, t }) => { const categories = [ { id: 'tomato', name: 'Tomato', emoji: '🍅' }, { id: 'potato', name: 'Potato', emoji: '🥔' }, { id: 'corn', name: 'Corn', emoji: '🌽' }, { id: 'others', name: 'Others', emoji: '🌿' }]; return ( <div className="w-full max-w-md mx-auto bg-white rounded-2xl shadow-xl p-8 z-10 text-center"><h2 className="text-3xl font-bold text-gray-800 mb-6">{t.selectCrop}</h2><div className="grid grid-cols-2 gap-4">{categories.map(cat => ( <button key={cat.id} onClick={() => onSelectCategory(cat.id)} className="p-4 bg-gray-100 rounded-xl hover:bg-green-100 hover:shadow-lg transition-all transform hover:scale-105"><div className="text-6xl mb-2">{cat.emoji}</div><p className="font-bold text-gray-700">{cat.name}</p></button>))}</div><button onClick={onLogout} className="mt-8 text-sm text-gray-500 font-semibold hover:text-red-600 transition-colors flex items-center justify-center mx-auto"><LogoutIcon /> {t.logout}</button></div> ); };
  ...
  const handleImageChange = async (event) => {
    ...
    try {
      ...
      const transformedResult = {
        status: resultData.details.status,
        confidence: parseFloat(resultData.confidence),
        diseaseName: resultData.details.diseaseName,
        cause: resultData.details.cause,
        treatment: resultData.details.treatment
      };
  ...
  const handleSave = () => {
    ...
    const savePromise = new Promise(async (resolve, reject) => {
        try {
            ...
            let categoryForIcon = 'others';
            if (predictedClass.toLowerCase().includes('tomato')) categoryForIcon = 'tomato';
            if (predictedClass.toLowerCase().includes('potato')) categoryForIcon = 'potato';
            if (predictedClass.toLowerCase().includes('corn')) categoryForIcon = 'corn';
